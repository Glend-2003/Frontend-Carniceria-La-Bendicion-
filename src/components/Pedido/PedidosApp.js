...
const PedidosApp = () => {
  ...
  const fetchPedidos = async () => {
    try {
      setLoading(true);
      setRefreshing(true);
      const response = await axios.get("https://backend-carniceria-la-bendicion-production.up.railway.app/pedido/");
  ...
  const aplicarFiltrosAvanzados = async () => {
    try {
      ...
      let url = `https://backend-carniceria-la-bendicion-production.up.railway.app/pedido/filtrar?idUsuario=0`;
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      await axios.post(
        `https://backend-carniceria-la-bendicion-production.up.railway.app/pedido/actualizar`,
        editFormData
      );
  ...
  const handleChangeEstadoEntrega = async (pedidoId, nuevoEstado) => {
    ...
    try {
      ...
      await axios.put(
        `https://backend-carniceria-la-bendicion-production.up.railway.app/pedido/actualizarEstadoPedido/${pedidoId}`,
        null,
        {
          params: {
